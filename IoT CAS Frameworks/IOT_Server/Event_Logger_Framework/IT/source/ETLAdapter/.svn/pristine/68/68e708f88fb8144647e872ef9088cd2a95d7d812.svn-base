        ...
        try 
        {
            if (metricMap.size() > 0) {
                ...
                for (int i = 0; i < set1.size(); i++) 
                {
                    ...
                    rs = stmt.executeQuery("Select service,subservice,hostgroup,resourceType,resourceSubType,resourceId from hostinfo where host='" + rData.getHost() + "' and resourcetype='server'");
                    while (rs.next()) 
                    {
