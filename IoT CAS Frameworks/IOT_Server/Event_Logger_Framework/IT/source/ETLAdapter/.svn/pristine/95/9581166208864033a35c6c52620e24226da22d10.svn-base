...
public class EventLogParser 
{
    ...
    String defaultKeyConstants[]=CEPEventMetricsMapping.getSzdefaultConstants();
     ...
     public EventLogParser(String CEPEvent, String metrics, Properties properties, String eventID) {
            ...
            int parseLen = parseToken.length;//total no of metrics in the event stream
             ...
             System.out.println("Customer ID In Default Parser==="+mp_string_to_map.get(defaultKeyConstants[3].trim()));
              Connection connection = ConnectionDAO.getConnection(mp_string_to_map.get(defaultKeyConstants[3].trim()));
                    ...
                    System.out.println("Query=="+"Select service,subservice,hostgroup,resourceType,resourceSubType,resourceId from hostinfo where lower(resourceId)=lower('" + mp_string_to_map.get(defaultKeyConstants[2]) + "')");
                    ResultSet rs = stmt.executeQuery("Select service,subservice,hostgroup,resourceType,resourceSubType,resourceId from hostinfo where lower(resourceId)=lower('" + mp_string_to_map.get(defaultKeyConstants[2]) + "')");
                    while (rs.next()) {
                    ...
                    //for (int j = 0; j < parseLen && (vfound == false); j++) {
                        ...
                        if (mp_string_to_map.containsKey(metricsToken[i])) {
                            ...
                            if (!(dmValue < 0)) {
                                ...
                                reportData.setTimestamp1(mp_string_to_map.get(defaultKeyConstants[13]));

                                reportData.setMetricType(Type+metricToken[1]);
                                reportData.setValue(""+staticMetricValue);
                                reportData.setCategory(Type+metricToken[1]+":"+mp_string_to_map.get(defaultKeyConstants[12]));
