...
import contextSetters.NetEventContext;
...
public class NetEventParser {
    ...
    HashMap<String, ReportData> metricValueMap = new HashMap<String, ReportData>();//availMetricValueMap
    HashMap<String, ReportData> availMetricValueMap = new HashMap<String, ReportData>();//availMetricValueMap
    private static Logger log = Logger.getLogger(ETLAdapter.class.getName());
    ETLAdapter etl = null;
    PersistReportData dataInsert = null;
    int insertStatus = 0;

    public NetEventParser(String CEPEvent, String metrics, Properties properties, String eventID) {
        ...
        try {
            ...
            dataInsert = new PersistReportData();
                 Map<String,String> mp_string_to_map=convertStringToMap(CEPEvent);
                    ...
                    hostName = mp_string_to_map.get(defaultKeyConstants[17].trim());
              
                    timestamp1 = mp_string_to_map.get(defaultKeyConstants[18]);
                    System.out.println("Net Parser Time Stamp="+timestamp1);
                    /*String df = "MM/dd/yy HH:mm:ss";
                    SimpleDateFormat smdf = new SimpleDateFormat(df);
                    Date d = smdf.parse(timestamp1);
                    timestamp1 = ((Long) d.getTime()).toString();
                     System.out.println("Timestamp  in Net Parser"+timestamp1);
            ...
            if (flag1 || flag2) {
                if (resourceId != null) {
                    ...
                    ResultSet rs = stmt.executeQuery("Select service,subservice,hostgroup,resourceType,resourceSubType,resourceId from hostinfo where lower(resourceId)=lower('" + resourceId + "')");
                    while (rs.next()) {
            ...
            if (availability.equalsIgnoreCase("0")) { //server is down
                ...
                log.debug("Down state of Network occured.");

            } else if (availability.equalsIgnoreCase("1")) { //server is up
                reportData = new ReportData();
                log.debug("Up state of Network again occuppied.");
            ...
            if (gotoContextFlag) {
                NetEventContext contextSetter = new NetEventContext(properties);
                availMetricValueMap = contextSetter.setContextsInMap(availMetricValueMap);
                contextSetter = null;
            }
            log.debug("Inside NETEvent parser availMetricValueMap .size..." + availMetricValueMap.size());
     ...
     public Map<String,String> convertStringToMap(String szCepEvent)
    {
        ...
        String splitByComma[]=szCepEvent.split(",");
            ...
            {
                log.info("splitByComma this string::"+splitByComma[i]);
