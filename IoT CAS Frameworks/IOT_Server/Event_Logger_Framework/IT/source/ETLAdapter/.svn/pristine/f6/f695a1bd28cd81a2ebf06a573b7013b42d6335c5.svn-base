...
public class PersistReportData {
        ...
        try {
            if (metricMap.size() > 0) {
                ...
                for (int i = 0; i < set1.size(); i++) {
                    ...
                    rs = stmt.executeQuery("Select service,subservice,hostgroup from hostinfo where lower(host)=lower('" + rData.getHost() + "')");
                    ...
                    rs = stmt.executeQuery("Select count(*) from servicemetrics where host='" + rData.getHost() + "' and timestamp1=" + startdate_milli + " and metrictype='" + rData.getMetricType() + "'");
                    log.debug("QueryCount==" + "Select count(*) from servicemetrics where host='" + rData.getHost() + "' and timestamp1=" + startdate_milli + " and metrictype='" + rData.getMetricType() + "'");
            ...
            for (int i = 0; i < set1.size(); i++) {
                try {
                    ...
                    if (rData.getValue().equalsIgnoreCase("DOWN") && !downReord) {
                        ...
                        System.out.println("Down Query====" + "Insert into  servicemetrics(host,hostgroup,service,subservice,timestamp1,category,"
                                + "metrictype,resourceType, resourceSubType,resourceId,eventid) values('" + rData.getHost() + "','" + rData.getHostGroup() 
                                + "','" + rData.getService() + "','" + rData.getSubService() + "'," + rData.getTimestamp1() + ",'" + rData.getCategory() 
                                + "ME'" + ",'" + "downtime" + "','" + rData.getResourceType() + "','" + rData.getResourceSubType() + "','" 
                                + rData.getResourceId() + "','" + rData.getEventID() + "')");
    ...
    public String generateQuery(HashMap metricMap, ReportData rData) {
        ...
        String szQuery = "insert into servicemetrics "
                ...
                + "metrictype,metricvalue,resourceType, resourceSubType, resourceId,eventID,SLA) values";
        ...
        try {
            if (metricMap.size() > 0) {
                ...
                if (rData.getValue().contains(":") || rData.getValue().contains("OK")) {
                    ...
                    long timeInMillis = System.currentTimeMillis() / 1000;
                    szQuery = szQuery + "('" + rData.getHost() + "','" + rData.getHostGroup() + "','" + rData.getService() + "','" + rData.getSubService() 
                            + "'," + rData.getTimestamp1() + ",'" + timeInMillis + "','" + rData.getCategory() + "'" + ",'" + rData.getMetricType() 
                            + "','" + rData.getValue() + "','" + rData.getResourceType() + "','" + rData.getResourceSubType() + "','" 
                            + rData.getResourceId() + "','" + rData.getEventID() + "','" + rData.getSLAvalue() + "')";
                    // System.out.println("Query String==="+szQuery);
                } else {
    ...
    public String generateQuery(HashMap metricMap) {
        //Insert into servicemetrics(host,hostgroup,service,subservice,timestamp1,timestamp2,category,metrictype,metricvalue,resourceType, resourceSubType, resourceId,eventid) values('" + rData.getHost() + "','" + hostGroupName + "','" + service1 + "','" + subService1 + "'," +  startdate_milli+","+enddate_milli+ ","+"'"+rData.getCategory()+"'"+",'"+rData.getMetricType()+"',"+rData.getValue()+"',"+rData.getResourceType()+"',"+rData.getResourceSubType()+"',"+rData.getResourceId()+"',"+rData.getEventID()+")
        String szQuery = "insert ignore into servicemetrics (host,hostgroup,service,subservice,timestamp1,timestamp2,category,metrictype,metricvalue,resourceType, resourceSubType, resourceId,eventID,SLA) values";
        int initialLen = szQuery.length();
        try {
            if (metricMap.size() > 0) {
                Set<String> set1 = null;
                ...
                for (int i = 0; i < set1.size(); i++) {
                    ...
                    if (rData.getValue().contains(":") || rData.getValue().contains("OK")) {
                        long timeInMillis = System.currentTimeMillis() / 1000;
                        if (i < (set1.size() - 1)) {
                            szQuery = szQuery + "('" + rData.getHost() + "','" + rData.getHostGroup() + "','" + rData.getService() + "','" + rData.getSubService() + "'," + rData.getTimestamp1() + ",'" + timeInMillis + "','" + rData.getCategory() + "'" + ",'" + rData.getMetricType() + "','" + rData.getValue() + "','" + rData.getResourceType() + "','" + rData.getResourceSubType() + "','" + rData.getResourceId() + "','" + rData.getEventID() + "','" + rData.getSLAvalue() + "'),";
                        } else {
                            szQuery = szQuery + "('" + rData.getHost() + "','" + rData.getHostGroup() + "','" + rData.getService() + "','" + rData.getSubService() + "'," + rData.getTimestamp1() + ",'" + timeInMillis + "','" + rData.getCategory() + "'" + ",'" + rData.getMetricType() + "','" + rData.getValue() + "','" + rData.getResourceType() + "','" + rData.getResourceSubType() + "','" + rData.getResourceId() + "','" + rData.getEventID() + "','" + rData.getSLAvalue() + "')";
                        }
        ...
        try {
            ...
            for (int i = 0; i < RDset.size(); i++) {
                try {
                    ...
                    String szQuery = "Select * from servicemetrics where metrictype='"+rData.getMetricType()+"' and host='" + rData.getHost()
                            + "' and resourceId='" + rData.getResourceId() + "'";
                    ...
                    if (rs1.next()) {
                        ...
                        szQuery = "Insert into  servicemetrics(host,timestamp1,category,metrictype,"
                                + "resourceType, resourceSubType,resourceId) values('" + rData.getHost() + "','" + rData.getTimestamp1() + 
                                "','" + rData.getCategory() + "'" + ",'" + rData.getMetricType() + "','" + rData.getResourceType() + "','" 
                                + rData.getResourceSubType() + "','" + rData.getResourceId() + "')";
