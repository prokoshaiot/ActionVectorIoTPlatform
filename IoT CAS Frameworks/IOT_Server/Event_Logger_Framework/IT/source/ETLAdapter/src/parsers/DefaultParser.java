...
public class DefaultParser {
    ...
    public void initialize(String CEPEvent, String metrics, Properties properties, String eventID) {
        ...
        try {
            ...
            System.out.println("Query==" + "Select service,subservice,hostgroup,resourceType,resourceSubType,resourceId,customerid from hostinfo where lower(resourceId)=lower('" + mp_string_to_map.get(defaultKeyConstants[2]) + "')");
            ResultSet rs = stmt.executeQuery("Select service,subservice,hostgroup,resourceType,resourceSubType,resourceId,customerid from hostinfo where lower(resourceId)=lower('" + mp_string_to_map.get(defaultKeyConstants[2]) + "')");
            while (rs.next()) {
                    ...
                    if (mp_string_to_map.containsKey(metricsToken[i])) {
                        ...
                        if (token[1] != null && token[1] != "null") {
                            ...
                            if (!(dmValue < 0)) {
                                ...
                                reportData.setCustomerID(customerid);
