...
//            stat = uniqueconnection.createStatement();
            ...
            if (resourceType.equals("'Desktop'") || resourceType.equals("'Inverter'")) {
                ...
                String szQuery = "select sum(case when timestamp1<" + smilli + " and timestamp2<" + smilli + " then " + (emilli - smilli) + " when timestamp1<" + smilli + " and timestamp2>" + smilli + " then timestamp2-" + smilli + " else timestamp2-timestamp1 end) as availmetricvalue,service,subservice,"
                        ...
                        + "from servicemetrics where timestamp2 between "
                        + smilli
                        + " and "
                        + emilli
...
//                stat = uniqueconnection.createStatement();
                ...
                while (rs.next()) {
                    ...
                    String downTimestamp = getDownTimeStamp(smilli, emilli, resourceType, host, resourceId, customer);
                    String availStatus = getAvailSatus(resourceType, host, resourceId, customer);
                    long SDate = smilli;
                    long EDate = emilli;
...
//            Statement st = uniqueconnection.createStatement();
     ...
     * *******************************************************************************
     */
    private static boolean getDownTimeWhenAdapterStop(long smilli, long emilli, String resourceType, String host, String service, String resourceId, long timestamp, String customer) throws IOException, SQLException {
