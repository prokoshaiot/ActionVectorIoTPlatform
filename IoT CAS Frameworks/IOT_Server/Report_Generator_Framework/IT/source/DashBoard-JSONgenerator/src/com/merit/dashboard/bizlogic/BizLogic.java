...
public class BizLogic {
     ...
     * **************************************************************************************************
     */
    private LinkedHashMap<String, String> getUniqueKeyForBusinessSLA(LinkedHashMap<String, String> mapServicemetric, String metricAvail) {
    ...
    private LinkedHashMap<String, String> getUniqueKeyForResourceTab(LinkedHashMap<String, String> mapServicemetric, String metricAvail) {
        LinkedHashMap<String, String> map_ServiceMetricAvail = new LinkedHashMap<String, String>();
        JSONObject temp = null;
        String tempKey = "";
        String mValue = "";
        ...
        LinkedHashMap<String, String> mapServicemetricForResourceTab = new LinkedHashMap<String, String>();
        Iterator keySetItr = mapServicemetric.keySet().iterator();
        while (keySetItr.hasNext()) {
            String keyString = keySetItr.next().toString();
            String keyValue = mapServicemetric.get(keyString);
            log.info("keyString==>>" + keyString);
            String newKeyString = keyString.replaceFirst(keyString.split(",")[0] + ",", "");
            log.info("new keyString==>>" + newKeyString);
            mapServicemetricForResourceTab.put(newKeyString, keyValue);
        }
        ...
        if (!metricAvail.equals("[]")) {
            try {
                ...
                arrJobj = new JSONArray(metricAvail);
                if (arrJobj.length() >= mapServicemetricForResourceTab.size()) {
                    for (int p = 0; p < arrJobj.length(); p++) {
                        temp = null;
                        temp = arrJobj.getJSONObject(p);
                        tempKey = "";
                        tempKey = temp.getString("ServerName") + "," + temp.getString("ResourceID");
                        mValue = null;
                        mValue = mapServicemetricForResourceTab.get(tempKey);
                        if (mValue != null) {
                            mTempJObj = new JSONObject("{" + mValue + "}");
                            if (temp.get("Health").equals("CRITICAL")) {
                                mTempJObj.accumulate("ResourceNames", "Availability");
                            ...
                            mTempJObj = new JSONObject("{" + "\"ResourceType\":\"" + temp.getString("ResourceType") + "\",\"ServerName\":\"" + temp.getString("ServerName") + "\",\"ResourceNames\":[\"Availability\"],\"Health\":[\"" + temp.getString("Health") + "\"],\"ResourceSubType\":[\"0\",\"1\"],\"ResourceChartType\":[\"0\",\"1\"],\"ResourceID\":\"" + temp.getString("ResourceID") + "\"" + "}");
                        }
                    ...
                    Iterator itr = mapServicemetricForResourceTab.entrySet().iterator();
                    String combineMetricAvail1 = "";
                    while (itr.hasNext()) {
                        String ResourceType = "";
                        String ServerName = "";
                        ...
                        for (int p = 0; p < arrJobj.length(); p++) {
                            temp = arrJobj.getJSONObject(p);
                            if (temp.get("ServerName").equals(mTempJObj.get("ServerName")) && temp.get("ResourceID").equals(mTempJObj.get("ResourceID"))) {
                                ResourceType = temp.get("ResourceType").toString();
                                ServerName = temp.get("ServerName").toString();
                                String rname = "";
                                if (!temp.get("ResourceNames").toString().replace("[", "").replace("]", "").equals("")) {
                        ...
                        combineMetricAvail = "\"ResourceType\":\"" + ResourceType + "\",\"ServerName\":\"" + ServerName + "\",\"ResourceNames\":[" + ResourceName + "],\"Health\":[" + Health + "],\"ResourceSubType\":[" + resourcesubtype + "],\"ResourceChartType\":" + resourceChartType + ",\"ResourceID\":\"" + ResourceID + "\"";
                        map_ServiceMetricAvail.put((String) pairs.getKey(), combineMetricAvail);
                    }
                }
            ...
            //System.out.println("BizLogic.getUniqueKey()    mapServicemetric = " + mapServicemetric);
            return mapServicemetricForResourceTab;
        }
        System.out.println("BizLogic.getUniqueKeyForBusinessSLA()    map_ServiceMetricAvail = " + map_ServiceMetricAvail);
    ...
    private String combineTwoMapForBusinessSLA(LinkedHashMap<String, String> serviceMetricInfo, LinkedHashMap<String, String> ticketInfo) {
        try {
    ...
    @SuppressWarnings("rawtypes")
    private String combineTwoMapForResourceTab(LinkedHashMap<String, String> serviceMetricInfo, LinkedHashMap<String, String> ticketInfo) {
        try {
            String szMetricTypeValueJson = "";
            String createJsonWithMap = "";
            String combinedCritical = "";
            ...
            LinkedHashMap<String, String> ticketInfoForResourceTab = new LinkedHashMap<String, String>();
            Iterator keySetItr = ticketInfo.keySet().iterator();
            while (keySetItr.hasNext()) {
                String keyString = keySetItr.next().toString();
                String keyValue = ticketInfo.get(keyString);
                String newKeyString = keyString.replaceFirst(keyString.split(",")[0] + ",", "");
                ticketInfoForResourceTab.put(newKeyString, keyValue);
            }
     ...
     * **************************************************************************************************
     */
    public String generateJsonFromCombinedTableForBusinessSLA(LinkedHashMap<String, String> serviceMetricInfo, LinkedHashMap<String, String> ticketInfo, String szAvailJson) {
        LinkedHashMap<String, String> map_UniqueKey = getUniqueKeyForBusinessSLA(serviceMetricInfo, szAvailJson);
        String szCombinedJson = combineTwoMapForBusinessSLA(map_UniqueKey, ticketInfo);
        map_UniqueKey = null;
        return szCombinedJson;
    }

    public String generateJsonFromCombinedTableForResourceTab(LinkedHashMap<String, String> serviceMetricInfo, LinkedHashMap<String, String> ticketInfo, String szAvailJson) {
        LinkedHashMap<String, String> map_UniqueKey = getUniqueKeyForResourceTab(serviceMetricInfo, szAvailJson);
        String szCombinedJson = combineTwoMapForResourceTab(map_UniqueKey, ticketInfo);
        map_UniqueKey = null;
        return szCombinedJson;
    }
