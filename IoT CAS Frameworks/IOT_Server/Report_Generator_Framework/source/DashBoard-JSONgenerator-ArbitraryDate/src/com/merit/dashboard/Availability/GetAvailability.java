...
public class GetAvailability {
    ...
    public static String getAvailabilityByPeriod(long emilli, long smilli,
        ...
        String szAvailJson = "[]";
        String custQuery = " and customerid in (select id from customerinfo where customername='" + cCustomer + "')" ;//and customername='" + cCustomer + "'"
...
//            stat = uniqueconnection.createStatement();
            ...
            if (resourceType.equals("'Desktop'")) {
                ...
                String szQuery = "select t1.availmetricvalue, t1.service, t1.subservice, t1.host, t1.metrictype, t1.category, t1.resourceId, t1.SLA, "
                        + "t1.resourceType, t2.customername from((select sum(case when timestamp1<" + smilli + " and timestamp2<" + smilli + " then " 
                        + (emilli - smilli) + " when timestamp1<" + smilli + " and timestamp2>" + smilli + " then timestamp2-" + smilli
                        + " else timestamp2-timestamp1 end) as availmetricvalue,service,subservice,"
                        + "host,metrictype,category,resourceId,sla as SLA,resourceType,customerid from servicemetrics where "
                        + "timestamp2 between " + smilli + " and " + emilli + " and service is not null  and service!='null' "
                        + "and timestamp2 is not null and metrictype='downtime' and category!='availabilityME' and resourcetype in (" + resourceType + ")";
                if (cCustomer != null) {
                    szQuery += custQuery;
                }
                if (service != null) {
                ...
                szQuery += " group by metrictype,host,customerid,service,subservice,category,resourceid,resourcetype,sla order by "
                        + "customerid,service,subservice,metrictype,host) t1 inner join customerinfo t2 on t1.customerid=t2.id)";
                
                /*
                 * select t1.availmetricvalue, t1.service, t1.subservice, t1.host, t1.metrictype, t1.category, t1.resourceId, t1.SLA, t1.resourceType, 
                 * t2.customername from((select sum(case when timestamp1<1429381800 and timestamp2<1427826600 then 1690561 when timestamp1<1427826600 and 
                 * timestamp2>1427826600 then timestamp2-1427826600 else timestamp2-timestamp1 end) as availmetricvalue,service,subservice,host,metrictype,
                 * category,resourceId,sla as SLA,resourceType,customerid from servicemetrics where timestamp2 between 1427826600 and 1429517161 and 
                 * service is not null  and service!='null' and timestamp2 is not null and metrictype='downtime' and resourcetype in ('Inverter') group by 
                 * metrictype,host,customerid,service,subservice,category,resourceid,resourcetype,sla order by customerid,service,subservice,metrictype,
                 * host) t1 inner join customerinfo t2 on t1.customerid=t2.id);
                 */
                log.debug("DashBoard Availability With in Time period /" + timestampselection + "/" + resourceType + "/ \n" + szQuery);
//                stat = uniqueconnection.createStatement();
                System.out.println("availability query=="+szQuery);
                rs = ConnectionDAO.executerQuery(szQuery, customer);
                String szHostList = "";
    ...
    private static String missedHostAvailability(long emilli,
        ...
        try {
            ...
            String dbservice;//
            while (rs3.next()) {
                System.out.println("in rs3==============="+finalJson);
                ...
                if (!szHostList.contains(host + "/" + sz_resourceType + "/" + resourceId)) {
                    ...
                    //}
                    System.out.println("sz_avail_json==============="+sz_avail_json);
                }
            }
            ConnectionDAO.closeStatement();
            // check for the host having downtime
            {
                ...
                float deveation;
                while (rs4.next()) {
                    System.out.println("in rs4==============="+finalJson);
                    ...
                    if (!szHostList.contains(host + "/" + sz_resourceType + "/" + resourceId)) {
                        ...
                        System.out.println("szOrginalDownTime==============="+szOrginalDownTime);
                        System.out.println("szTimeRequested==============="+szTimeRequested);
                        if (szOrginalDownTime < szTimeRequested) {
                            ...
                            sla_deviation = first;
                            System.out.println("szTimeRequested==============="+deveation);
                            szHealth = "CRITICAL";
                            if (deveation > 0) {
                                szHealth = "OK";
                            }
                            ...
                            System.out.println("sz_avail_json else==============="+sz_avail_json);
                        }
                    }
                }
                rs4 = null;//modified
...

                {
                    ...
                    while (rs5.next()) {
                        System.out.println("rs5==============="+sz_avail_json);
