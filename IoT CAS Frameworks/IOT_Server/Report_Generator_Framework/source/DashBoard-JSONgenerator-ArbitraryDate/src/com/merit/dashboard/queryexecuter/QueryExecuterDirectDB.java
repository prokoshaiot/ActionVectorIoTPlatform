 ...
 * *****************************************************************************************************
 */
public class QueryExecuterDirectDB {

    static Logger log = Logger.getLogger(QueryExecuterDirectDB.class);
//    Statement stat = null;
...
//            stat = uniqueconnection.createStatement();
             ...
             * ***************************************************************************************************
             */
            log.info("Ticket Query /" + resourceType + "/\n" + szQuery);
                ...
                if (selection.equals("CriticalAlerts")) {
                    ...
                    if (splitsummaryname.length > 0 && !splitsummaryname[0].equalsIgnoreCase("")) {
                        for (int b = 0; b < splitsummaryname.length; b++) {
                            log.info("splitsummaryname==>>" + splitsummaryname[b]);
                            if (resourceType.equals("server")) {
                                if (!splitsummaryname[b].toLowerCase().contains("fnsp") && !splitsummaryname[b].toLowerCase().contains("tem")) {
                                    //log.info("183>>>>splitsummaryname[b] = " + splitsummaryname[b] + ", DBUtilHelper.getMetrics_mapping_properties().getProperty(splitsummaryname[b]) = " + DBUtilHelper.getMetrics_mapping_properties().getProperty(splitsummaryname[b]).trim());
...
//            stat = uniqueconnection.createStatement();
            ...
            log.info("Report Metric Query /" + resourceType + "/ \n" + szQuery);
...
//            Connection dataBaseConnectionMetric = ConnectionDAO.getConnection(customer);
            ...
            try {
                ...
                for (int p = 0; p < arrJobj.length(); p++) {
                    ...
                    log.info("rekha  ServiceName==>>" + tempKey);
...
//            Connection dataBaseConnectionTicket = ConnectionDAO.getConnection(customer + "");
            ...
            cquery = "select distinct service,customerid from hostinfo where customerid=(select id from customerinfo where customername='" + cCustomer
                    + "'" + ");";
...
//            Connection dataBaseConnectionMetric = ConnectionDAO.getConnection(customer);
            ...
            serviceMetricInfo = modifyHashMap(serviceMetricInfo);
            log.debug("Default Service Metric Query /" + timestampselection + "/ \n" + szQueryMetric);
            log.info("Query for customers left grid==>>" + szQueryMetric);
            ...
            try {
                ...
                for (int p = 0; p < arrJobj.length(); p++) {
                    ...
                    log.info("rekha  Customer==>>" + tempKey);
                    //if (!(tempKey.equals("null"))){
                    if (serviceMetricInfo.get(tempKey) == null || temp.getString("Health").equals("CRITICAL")) {
...
//            dataBaseConnectionTicket = null;
    ...
    public String generateDefaultResourceTypeJson(String resourceType, String cCustomer, String service,
        ...
        log.info("Returning defaultResourceType.json==>>" + szResourceTypeJson);
...
//            Statement stat2 = dataBaseConnection.createStatement();
            ...
            while (rs4.next()) {
                ...
                for (int i = 0; i < headingNames.length - 1; i++) {
                    log.info("headingNames[i]==>>" + headingNames[i]);
                    if (!headingNames[headingNames.length - 1].equals("Alert")) {
                        log.info("rs4.getString(headingNames[i])==>>" + rs4.getString(headingNames[i]));
...
//            stat = uniqueconnection.createStatement();
            ...
            log.info("QueryExecutor.collectMetricdata1()    serviceMetricInfo = " + serviceMetricInfo);
    ...
    public String getWatchDogAlertJson(String customer, String cCustomer) {
        ...
        log.info("WatchDog Query:====" + watchDogQuery);
        String watchdogJson = "[";
        try {
            ...
            if (rs4 == null) {
                log.info("822 rs4 null;");
            } else {
                log.info("822 rs4 is not null;");
            }
            String resourceType = "";
            String metrictype = "";
            String timestamp = "";
...
//            stat = uniqueconnection.createStatement();
             ...
             * ***************************************************************************************************
             */
            log.info("Ticket Query /" + cCustomer + "/\n" + szQuery);
...
//            stat = uniqueconnection.createStatement();
    ...
    public String generateWeatherJson(String resourceType, String cCustomer, String service,
        ...
        try {
            ...
            String zWeatherJSON = createWeatherJSONFromHashMap(serviceMetricInfo, cCustomer, service, "Sensor");
    ...
    public String generateCurrentSnapShotJson(String resourceType, String cCustomer, String service, String startDate, String endDate, String timestampselection, String customer) {
        try {
            ...
            log.debug("ResourceType in generateCurrentSnapShotJson is " + resourceType);
            if (service.equals("")) {
                aggQuery = "(select resourceid,metrictype,min(metricvalue) as minmetricvalue, max(metricvalue) as maxmetricvalue, round(avg(metricvalue)) "
                        + "as avgmetricvalue from servicemetrics where resourcetype=service and metrictype in ('DayEnergy','YearEnergy','TotalEnergy',"
                ...
                curQuery = "(select t.resourceid,t.metricvalue, t.metrictype from(select resourceid,customerid,service,resourcetype,metrictype,"
                        + "max(timestamp1) as MaxTime from servicemetrics where resourcetype=service and metrictype in ('DayEnergy','YearEnergy',"
                        + "'TotalEnergy','ACPower') and resourceid = service and customerid=(select id from customerinfo where customername='"
                ...
                log.debug("CurrentSnapShot Query at customer level /" + timestampselection + "/ \n" + szQueryMetric);
                ...
                aggQuery = "(select metrictype,min(metricvalue) as minmetricvalue, max(metricvalue) as maxmetricvalue, round(avg(metricvalue)) as "
                        + "avgmetricvalue from servicemetrics where resourcetype='" + service + "' and metrictype in ('DayEnergy','YearEnergy',"
                        + "'TotalEnergy','ACPower') " + "and resourceid = service and service='" + service + "' and customerid=(select id from "
                ...
                curQuery = "(select t.metricvalue, t.metrictype from(select customerid,service,resourcetype,metrictype,max(timestamp1) as MaxTime from "
                        + "servicemetrics where resourcetype=service and metrictype in ('DayEnergy','YearEnergy','TotalEnergy','ACPower') and "
                ...
                aggQuery = "(select resourceid,metrictype,min(metricvalue) as minmetricvalue,max(metricvalue) as maxmetricvalue,round(avg(metricvalue)) "
                        + "as avgmetricvalue from servicemetrics where resourcetype='" + resourceType + "' and metrictype in ('DayEnergy','YearEnergy',"
                        + "'TotalEnergy','ACPower','ACCurrent','ACFrequency','DCCurrent', 'DCVoltage') and resourceid != service and service='" + service
                ...
                curQuery = "(select t.resourceid,t.metricvalue, t.metrictype from(select customerid,service,resourcetype,resourceid,metrictype,"
                        + "max(timestamp1) as MaxTime from servicemetrics where resourcetype='" + resourceType + "' and metrictype in ('DayEnergy',"
                ...
                headingNamesMetric1 = new String[]{"ResourceID", "MetricType", "MetricValue", "MinMetricValue", "MaxMetricValue", "AvgMetricValue"};
                log.info("Device level snapshot query==for " + timestampselection + "==" + szQueryMetric);
            }
            ...
            curQuery = null;
            aggQuery = null;
            log.info("CurrentSnapShot json hashmap==>>" + this.serviceMetricInfo.toString());
            ...
            if ((!resourceType.equals("")) || (service.equals(""))) {
                ...
                while (entryItr.hasNext()) {
                    ...
                    tempHM = fillHashMap(tempHM, timestampselection, customer, cCustomer, service);
                    zCurrentSnapShotJSON = zCurrentSnapShotJSON + "{\"ResourceID\":\"" + szMapEntry.getKey() + "\",\n\"Value\":";
                    zCurrentSnapShotJSON = zCurrentSnapShotJSON + createCurrentSnapShotJSONFromHashMap(tempHM, cCustomer, service, service) + "},";
                }
                ...
                this.serviceMetricInfo = fillHashMap(this.serviceMetricInfo, timestampselection, customer, cCustomer, service);

                zCurrentSnapShotJSON = createCurrentSnapShotJSONFromHashMap(this.serviceMetricInfo, cCustomer, service, service);
            }
    ...
    private String createWeatherJSONFromHashMap(LinkedHashMap hm, String cCustomer, String service, String resourcetype) {
        String requestURL;
        URL url = null;
                ...
                while ((res = br.readLine()) != null) {
                    ...
                    if (jsonString != null) {
                        if (!(jsonString.equalsIgnoreCase("null"))) {
                            log.debug("response from getMetricUOM::" + jsonString);
    ...
    private String createCurrentSnapShotJSONFromHashMap(LinkedHashMap hm, String cCustomer, String service, String resourcetype) {
        HttpURLConnection con = null;
        BufferedReader br = null;
        String requestURL;
        URL url;
        try {
            ...
            while (entryItr.hasNext()) {
                ...
                log.info("cur-value=" + curVal + "min-value=" + minVal + "max-value=" + maxVal + "avg-value" + avgVal);
    ...
    private LinkedHashMap fillHashMap(LinkedHashMap<String, String> serviceMetricInfo, String timestampselection, String customer, String cCustomer, String service) {
        String periodMetric = "XXXEnergy";
        String dcPeriodEnergy = "DCXXXEnergy";
        String pr = "";
        String eprp = "";
        String co2saving = "";
        String earning = "";
        try {
            if (serviceMetricInfo.get("DayEnergy") == null) {
                serviceMetricInfo.put("DayEnergy", "\"MetricType\":\"DayEnergy\",\"MetricValue\":\"\"");
            }
            ...
            dcPeriodEnergy = dcPeriodEnergy.replace("XXX", timestampselection);
            ...
            if (szTemp != null) {
                ...
                int endIndx = szTemp.indexOf("\",", startIndx);
                if (endIndx != -1) {
                    szTemp = szTemp.substring(startIndx, endIndx);
                } else {
                    szTemp = null;
                }
                ...
                if ((szTemp != null)
                        && (!szTemp.equals(""))) {
                    Double periodEnergy = Double.valueOf(Double.parseDouble(szTemp));

                    String confVal = ResourceConfiguration.getConfValue(customer, cCustomer, service, "Default", null, dcPeriodEnergy);
                    if (confVal != null) {
                    ...
                    confVal = ResourceConfiguration.getConfValue(customer, cCustomer, service, "Default", null, "CO2Factor");
                    if (confVal != null) {
                        Double confDVal = Double.valueOf(Double.parseDouble(confVal));
                        Double val = Double.valueOf(periodEnergy.doubleValue() * confDVal.doubleValue());
                        DecimalFormat df = new DecimalFormat("#.##");
                    ...
                    confVal = ResourceConfiguration.getConfValue(customer, cCustomer, service, "Default", null, "Earnings/kWh");
                    if (confVal != null) {
                        Double confDVal = Double.valueOf(Double.parseDouble(confVal));
                        Double val = Double.valueOf(periodEnergy.doubleValue() * confDVal.doubleValue());
                        DecimalFormat df = new DecimalFormat("#.##");
