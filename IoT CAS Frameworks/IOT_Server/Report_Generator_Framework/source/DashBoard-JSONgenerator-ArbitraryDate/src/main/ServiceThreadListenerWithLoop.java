...
import com.merit.dashboard.ControllerWithLoop;
import com.merit.dashboard.DBUtil.DBUtilHelperWithLoop;
 ...
 * @author anandkv
 */
public class ServiceThreadListenerWithLoop extends Thread {

    static Logger log = Logger.getLogger(ServiceThreadListenerWithLoop.class);
    ...
    static ControllerWithLoop controler = new ControllerWithLoop();
    ...
    public static void initStandardFreq() {
        gHTimeWait = gHTimeWait * Long.parseLong(DBUtilHelperWithLoop.hourSleep);
        gDTimeWait = gDTimeWait * Long.parseLong(DBUtilHelperWithLoop.daySleep);
        gWTimeWait = gWTimeWait * Long.parseLong(DBUtilHelperWithLoop.weekSleep);
        gMTimeWait = gMTimeWait * Long.parseLong(DBUtilHelperWithLoop.monthSleep);
        gYTimeWait = gYTimeWait * Long.parseLong(DBUtilHelperWithLoop.yearSleep);
    }

    public static void main(String[] args) {
        try {
            List<String> al_customer_list;//
            String[] types;//
            ServiceThreadListenerWithLoop scl;//
            for (String arg : args) {
            ...
            new DBUtilHelperWithLoop();
            PropertyUtil.createMapFromProperty();
            System.out.println(" Context Listener Started");
            initStandardFreq();
            if (DBUtilHelperWithLoop.generateAllJSONForced != null) {
                if (DBUtilHelperWithLoop.generateAllJSONForced.trim().equalsIgnoreCase("false")) {
                    forcedJSON = false;
                }
            }

            al_customer_list = controler.getCustomerList();
            System.out.println("Im calling generateCustomerServiceMap");
            ControllerWithLoop.generateCustomerServiceMap(al_customer_list);
            ...
            types = DBUtilHelperWithLoop.getMetrics_mapping_properties().getProperty("ResourceName").split(",");
             ...
             */
            scl = new ServiceThreadListenerWithLoop();
    ...
    private static HashMap<String, String> createJSONMappingFolders(String resourceType) {
        String szProjectName = DBUtilHelperWithLoop.getMetrics_mapping_properties().getProperty("projectName");
        //String tomcat_home = System.getProperty("catalina.base");
        String tomcat_home = DBUtilHelperWithLoop.dashBoardJSONPATH;
    ...
    public static String getJSONMetricGroupSet(String resourceType, String selection) {
        ...
        /*if (resourceType.equalsIgnoreCase("server")) {
         ...
         return null;*/
        Object szGS = resourceMetricMap.get(resourceType).get(selection);
        if (szGS == null){
            return null;
        }else{
            return szGS.toString();
        }
    }
