...
public class DAOHelper
{
	...
	public String getMetricTypeWithTimestamp(String sz_customerID,String sz_metricType,String hostName,String sz_resourceType,String sz_selected_combo,String sz_tabName,String sz_interval,String resourceid)
     {
		String summaryJson="";
		...
		if (!(sz_concat_combo.contains("Alerts"))) {
         ...
         try
         {
			...
			if(sz_concat_combo.contains("Availability")){
				...
				//szQueryString = "select host,service,subservice,to_timestamp(timestamp1)::timestamp at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Timestamp")+" as downtime,to_timestamp(timestamp2)::timestamp at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Timestamp")+" as uptime,metrictype,case when metricvalue is null then 'NaN' else metricvalue end as metricvalue,category,resourceid,sla from servicemetrics where metrictype in ('"+new PropertyUtil().getreverseMapping_Properties().get("Availability") +"') and host='"+hostName.trim()+"'"+" and resourcetype='"+sz_resourceType.trim()+"' and (timestamp1>"+smilli+" or timestamp1=1)";
				szQueryString = "select host,service,subservice,to_char(to_timestamp(timestamp1) at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_timestamp")+",'yyyy-mm-dd HH24:mi:ss')  as downtime,to_char(to_timestamp(timestamp2) at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_timestamp")+",'yyyy-mm-dd HH24:mi:ss') as uptime,metrictype,case when metricvalue is null then 'NaN' else (timestamp2-timestamp1)::text end as metricvalue,category,resourceid,sla from servicemetrics where metrictype in ('"+propertyUtil.getreverseMapping_Properties().get("Availability") +"') and host='"+hostName.trim()+"'"+" and resourcetype='"+sz_resourceType.trim()+"' and  resourceid='"+resourceid+"' and (timestamp2>"+smilli+" or timestamp2 is null)";
				...
				uniqueconnection=ConnectionDAO.getConnection(sz_customerID+"_177");
				 if(sz_selected_combo.contains("Assignee"))
		           {
					 szQueryString="select task_id as TicketID,g2.attributes6 as Host ,g1.assignee as Assignee,task_summary as Summary,attributes4 as Service,created_date at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Date")+" as CreatedDate,g2.attributes7 as SubService,g2.status as Status ,g2.priority as Priority from gatask g2 ,gatasktypeassignee g1 where  created_date between '" + ar_dates[1] + "' and '" + ar_dates[0] + "'" + " and g1.taskid=g2.task_id  and g2.attributes10='"+sz_resourceType.trim()+ "' and g1.assignee='"+sz_metricType+"' and g2.attributes6='"+hostName.trim()+"' and g2.attributes12='"+resourceid+"'";
					 ...
					 summaryJson=generateJsonFromGivenQuery(szQueryString, sz_resourceType,sz_interval, sz_customerID, "SummaryPopup", headingNames);
		        	   ...
		        	   szQueryString="select task_id as TicketID,attributes6 as Host,attributes4 as Service,task_summary as Summary,created_date at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Date")+" as CreatedDate,attributes7 as SubService,status as Status,priority as Priority from gatask where attributes10 ='"+sz_resourceType.trim()+"' and attributes6='"+hostName.trim()+"' and created_date between '" + ar_dates[1] + "' and '" + ar_dates[0] + "'  and status='"+sz_metricType+"' and attributes12='"+resourceid+"'" ;
		           ...
		           else
		           {
		        	 ...
		        	 szQueryString="select task_id as TicketID,attributes6 as Host,attributes4 as Service,task_summary as Summary,created_date at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Date")+" as CreatedDate,attributes7 as SubService,status as Status,priority as Priority from gatask where attributes10 ='"+sz_resourceType.trim()+"' and attributes6='"+hostName.trim()+"' and created_date between '" + ar_dates[1] + "' and '" + ar_dates[0] + "'  and task_summary like'%"+sz_SummaryName.replaceAll("'", "")+"%' and attributes12='"+resourceid+"'" ;
		        	 String[]  headingNames = {"TicketID","Host","Service","SubService","CreatedDate","Summary","Status","Priority"};
		        	 summaryJson=generateJsonFromGivenQuery(szQueryString, sz_resourceType,sz_interval, sz_customerID, "SummaryPopup", headingNames);
		           }
			}
			else{
				szQueryString = "select host as Host,service as Service,subservice as SubService,to_char(to_timestamp(timestamp1) at time zone "+DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_timestamp")+",'yyyy-mm-dd HH24:mi:ss') as Timestamp,metrictype as MetricType,case when metricvalue is null then 'NaN' else round(cast(metricvalue as numeric),2) end as MetricValue,category as Description,resourceid,sla from servicemetrics where metrictype in ('"+propertyUtil.getreverseMapping_Properties().get(sz_metricType)+"') and host='"+hostName.trim()+"'"+" and resourcetype='"+sz_resourceType.trim()+"' and  resourceid='"+resourceid+"' and timestamp1 between "+smilli +" and "+ emilli;
	...
	public String generateJsonFromGivenQuery(String szQueryString,String resourceType,String timestampselection, String customer,String jsonnames,String[] headingNames){
		...
		try{
			...
			PdfPCell cell = new PdfPCell(new Paragraph("Summary"));
		   ...
		   /** apply custom font style*/
			...
			sheet.addMergedRegion(new Region(0, (short) 0, 0,(short) centerOfSheet));
			style.setAlignment(CellStyle.ALIGN_CENTER);
			cellMgHead.setCellStyle(style);
			HSSFRichTextString szHeader=null;
			for(int p=0;p<headingNames.length;p++){
				table.addCell(headingNames[p]); /** PDF Header String*/
			...
			ResultSet rs=null;
			rs = stat.executeQuery(szQueryString);
			int rowindex=1;
