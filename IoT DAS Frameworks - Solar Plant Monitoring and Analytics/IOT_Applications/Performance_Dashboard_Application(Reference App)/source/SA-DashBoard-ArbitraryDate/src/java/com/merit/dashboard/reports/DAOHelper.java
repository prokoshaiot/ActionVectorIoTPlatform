...
public class DAOHelper {
    ...
    public String getMetricTypeWithTimestamp(String sz_customerID, String sz_metricType, String hostName, String sz_resourceType,
            String sz_selected_combo, String sz_tabName, String sz_interval, String resourceid, String cCustomer, String service, String szDate) {
        String summaryJson = "";
        PropertyUtil propertyUtil = null;
        String ar_dates[] = DateGenerator.dateGenerator(sz_interval, szDate);
        ...
        try {
            ...
            String szMetricCustFilter = " and customerid=(select id from "
                    + "customerinfo where customername='" + cCustomer + "')";
            String szMetricServiceFilter = " and service = '" + service + "'";
            String szTicketCustFilter = " and customerid='" + cCustomer + "'";
            String szTicketServiceFilter = " and attributes4='" + service + "'";

            if (sz_concat_combo.contains("Availability")) {
                szQueryString = "select host,service,subservice,to_char(to_timestamp(timestamp1) at time zone "
                        + DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_timestamp")
                        + ",'yyyy-mm-dd HH24:mi:ss')  as downtime,to_char(to_timestamp(timestamp2) at time zone "
                        + DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_timestamp")
                        + ",'yyyy-mm-dd HH24:mi:ss') as uptime,metrictype,case when metricvalue is null then 'NaN' "
                        + "else (timestamp2-timestamp1)::text end as metricvalue,category,resourceid,sla from servicemetrics "
                        + "where metrictype in ('" + propertyUtil.getreverseMapping_Properties().get("Availability") + "') "
                        + "and host='" + hostName.trim() + "'" + " and resourcetype='" + sz_resourceType.trim() + "' and  resourceid='"
                        + resourceid + "' and (timestamp2>" + smilli + " or timestamp2 is null)";
                if (cCustomer != null) {
                    if (!(cCustomer.equals("null")) && !(cCustomer.equals(""))) {
                        szQueryString += szMetricCustFilter;
                    }
                }
                if (service != null) {
                    if (!(service.equals("null")) && !(service.equals(""))) {
                        szQueryString += szMetricServiceFilter;
                    }
                }
                ...
                if (sz_selected_combo.contains("Assignee")) {
                    szQueryString = "select task_id as TicketID,g2.attributes6 as Host ,g1.assignee as Assignee,task_summary as Summary"
                            + ",attributes4 as Service,created_date at time zone "
                            + DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Date")
                            + " as CreatedDate,g2.attributes7 as SubService,g2.status as Status ,g2.priority as Priority from gatask g2 "
                            + ",gatasktypeassignee g1 where  created_date between '" + ar_dates[1] + "' and '" + ar_dates[0] + "'"
                            + " and g1.taskid=g2.task_id  and g2.attributes10='" + sz_resourceType.trim() + "' and g1.assignee='"
                            + sz_metricType + "' and g2.attributes6='" + hostName.trim() + "' and g2.attributes12='" + resourceid + "'";
                    if (cCustomer != null) {
                        if (!(cCustomer.equals("null")) && !(cCustomer.equals(""))) {
                            szQueryString += szTicketCustFilter;
                        }
                    }
                    if (service != null) {
                        if (!(service.equals("null")) && !(service.equals(""))) {
                            szQueryString += szTicketServiceFilter;
                        }
                    }
                    ...
                    szQueryString = "select task_id as TicketID,attributes6 as Host,attributes4 as Service,task_summary as Summary,"
                            + "created_date at time zone "
                            + DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_Date")
                            + " as CreatedDate,attributes7 as SubService,status as Status,priority as Priority from gatask where "
                            + "attributes10 ='" + sz_resourceType.trim() + "' and attributes6='" + hostName.trim()
                            + "' and created_date between '" + ar_dates[1] + "' and '" + ar_dates[0] + "'  and status='" + sz_metricType
                            + "' and attributes12='" + resourceid + "'";
                    if (cCustomer != null) {
                        if (!(cCustomer.equals("null")) && !(cCustomer.equals(""))) {
                            szQueryString += szTicketCustFilter;
                        }
                    ...
                    szQueryString = "select task_id as TicketID,attributes6 as Host,attributes4 as Service,task_summary as Summary,"
                            ...
                            + " as CreatedDate,attributes7 as SubService,status as Status,priority as Priority from gatask where "
                            + "attributes10 ='" + sz_resourceType.trim() + "' and attributes6='" + hostName.trim()
                            + "' and created_date between '" + ar_dates[1] + "' and '" + ar_dates[0] + "'  and task_summary like'%"
                            + sz_SummaryName.replaceAll("'", "") + "%' and attributes12='" + resourceid + "'";
                    if (cCustomer != null) {
                        if (!(cCustomer.equals("null")) && !(cCustomer.equals(""))) {
                            szQueryString += szTicketCustFilter;
                        }
                ...
                szQueryString = "select host as Host,service as Service,subservice as SubService,to_char(to_timestamp(timestamp1) at "
                        + "time zone " + DBUtilHelper.getMetrics_mapping_properties().getProperty("ServerTimeZone_timestamp")
                        + ",'yyyy-mm-dd HH24:mi:ss') as Timestamp,metrictype as MetricType,case when metricvalue is null then 'NaN' "
                        + "else round(cast(metricvalue as numeric),2) end as MetricValue,category as Description,resourceid,sla from "
                        + "servicemetrics where metrictype in ('" + propertyUtil.getreverseMapping_Properties().get(sz_metricType)
                        + "') and host='" + hostName.trim() + "'" + " and resourcetype='" + sz_resourceType.trim() + "' and  "
                        + "resourceid='" + resourceid + "' and timestamp1 between " + smilli + " and " + emilli ;
                if (cCustomer != null) {
                    if (!(cCustomer.equals("null")) && !(cCustomer.equals(""))) {
                        szQueryString += szMetricCustFilter;
                    }
                ...
                szQueryString+= " order by timestamp1";
                System.out.println("getMetricTypeWithTimestamp query==>>" + szQueryString);
